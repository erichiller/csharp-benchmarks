...
public struct ManualResetValueTaskSourceCore2<TResult> {
    ...
    public void OnCompleted( Action<object?> continuation, object? state, short token, ValueTaskSourceOnCompletedFlags flags ) {
        ...
        if ( ( flags & ValueTaskSourceOnCompletedFlags.UseSchedulingContext ) != 0 ) {
            if ( SynchronizationContext.Current is { } sc &&
                ...
                _capturedContext = _capturedContext is null ? sc : new CapturedSchedulerAndExecutionContext( sc, ( ExecutionContext )_capturedContext );
            } else {
                TaskScheduler ts = TaskScheduler.Current;
                if ( ts != TaskScheduler.Default ) {
                    _capturedContext = _capturedContext is null ? ts : new CapturedSchedulerAndExecutionContext( ts, ( ExecutionContext )_capturedContext );
                }
            }
        }
    ...
    private void SignalCompletion( ) {
        if ( _completed ) {
            ThrowHelper.ThrowInvalidOperationException($"_completed is {_completed}");
        }
        _completed = true;

        Debug.Assert(_continuation != ManualResetValueTaskSourceCoreShared.s_sentinel, $"The continuation was the completion sentinel.");
        Console.WriteLine("1111111111111111111111111111111111111111111111111111111");
...
internal static class ManualResetValueTaskSourceCoreShared // separated out of generic to avoid unnecessary duplication
{
    ...
    {
        const string failMsg = "The sentinel delegate should never be invoked.";
        Debug.Fail( failMsg );
        ThrowHelper.ThrowInvalidOperationException( failMsg );
    }
    ...
    //     ExecutionContext? currentContext = Thread.CurrentThread.ExecutionContext;
    //
...
internal static class ThrowHelper {
    [ DoesNotReturn ]
    internal static void ThrowInvalidOperationException( string? msg = null ) => throw new InvalidOperationException( msg );
}