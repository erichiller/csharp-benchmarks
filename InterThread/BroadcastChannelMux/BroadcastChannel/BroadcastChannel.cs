...
public class BroadcastChannel<TData, TResponse> : IDisposable where TResponse : IBroadcastChannelResponse {
    ...
    // internal void RemoveReader( BroadcastChannelReader<TData, TResponse> reader ) => this.Writer?.RemoveReader( reader );
    ...
    public override string ToString( ) => //this.GetType().GenericTypeShortDescriptor( useShortGenericName: false )} // TODO: RESTORE
        this.GetType().Name + " [Hash: {this.GetHashCode()}] [Writer: {this._writer}]";
}
