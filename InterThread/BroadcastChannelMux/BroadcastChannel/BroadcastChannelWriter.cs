...
public class BroadcastChannelWriter<TData, TResponse> : ChannelWriter<TData>, IDisposable where TResponse : IBroadcastChannelResponse {
    ...
    private void removeReader( in int writerHash ) {
        ...
        if ( writerFound is null ) {
            throw new Exception( $"ChannelWriter with hash {writerHash} was not found." ); // TODO: more exact Exception here
        }
    }
    ...
    public override string ToString( ) {
        lock ( this._readersLock ) {
            return $"{this.GetType().GenericTypeShortDescriptor( useShortGenericName: false )} [Hash: {this.GetHashCode()}] [Readers ({ReaderCount}): {this._outputWriters.Select( r => r.ToString() ).ToCommaSeparatedString()}]";
        }
    }
}
