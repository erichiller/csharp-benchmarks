...
public class GrpcReceiveWorker : BackgroundService {
    ...
    protected override async Task ExecuteAsync( CancellationToken stoppingToken ) {
        ...
        while ( !stoppingToken.IsCancellationRequested ) {
            ...
            await foreach ( var message in _counterService.ServerToClientStream( new CounterRequest() ).WithCancellation(stoppingToken) ) {
                _logger.LogInformation( "Incrementing count by {Count}", message.Count );
            }
